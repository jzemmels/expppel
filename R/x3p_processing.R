...
#' @note The x3p_ransac function will throw an error if the final
...
x3p_ransac <- function(x3p,
  ...
  estimatedBreechFace <- x3p_levelBF(ransacFit = ransacFit,
...
x3p_detectFPCircle <- function(surfaceMat,
...
x3p_remove_fp_circle <- function(x3p,
                                      aggregationFunction = mean,
                                      smootherSize = 2*round((.1*nrow(surfaceMat)/2)) + 1,
                                      gridSize = 40,
                                      gridGranularity = 1,
  ...
  fpImpressionCircle <- x3p_detectFPCircle(surfaceMat = surfaceMat,
                                                  aggregationFunction = aggregationFunction,
...
x3p_gaussian_filter <- function(x3p,
                                   wavelength = c(16,500),
                                   filtertype = "bp"){
...
# @param croppingThresh argument for cmcR::x3p_cropWS
...
x3p_cropExterior <- function(x3p,
  ...
  x3p <- x3p_cropWS(x3p,croppingThresh = croppingThresh)
...
x3p_filterInterior <- function(x3p,
                                      scheme = 3,
                                      high_connectivity = FALSE,
                                      tolerance = 0,
                                      radiusOffset = 0){
...
#'                                        region = "exterior")
#'
#' fadul1.1_extIntCropped <- x3p_crop(x3p = fadul1.1_extCropped,
#'                                           radiusOffset = 200,
#'                                           region = "interior")
#'
#' x3pListPlot(list("Original" = fadul1.1,
...
# helper function for x3p_erodePrimer and x3p_dilateFP functions
...
x3p_dilateFP <- function(x3p,dilationRadius = 50){
...
x3p_erodePrimer <- function(x3p,erosionRadius = 50){
...
#'
#'@export
x3p_erode <- function(x3p,region,morphRadius = 50){
  ...
  if(region == "interior"){
    ...
    return(x3p_dilateFP(x3p = x3p,dilationRadius = morphRadius))
  }
  if(region == "exterior"){

    return(x3p_erodePrimer(x3p = x3p,erosionRadius = morphRadius))
